...
public class PatientDisplayModern extends JPanel {
    ...
    public PatientDisplayModern() {
        ...
        styleButton(backBtn, new Color(220, 53, 69)); // Red
        ...
        refreshBtn = new JButton("⟳ Refresh"); // Icon-like text or you can use an actual icon
        styleButton(refreshBtn, new Color(0, 123, 255)); // Primary blue
        refreshBtn.setPreferredSize(new Dimension(110, 32)); // Wider button
        refreshBtn.setFont(new Font("Segoe UI Semibold", Font.PLAIN, 13));
        refreshBtn.setFocusPainted(false);
//refreshBtn.setBorder(new RoundBorder(new Color(0, 123, 255))); // Optional rounded effect
        ...
        // Table model with ImageIcon for image column
        ...
        // Image preview tooltip on hover for image column
        ...
        searchField.addKeyListener(new KeyAdapter() {
            public void keyReleased(KeyEvent e) {
                if (searchField.getText().trim().isEmpty()) {
                    patientData = new ArrayList<>(originalData);
                    page = 1;
                    displayPage();
                } else {
                    filterAndSort();
    ...
    private void loadDataFromDatabase() {
        patientData.clear();
        originalData.clear();
        try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital_db", "root", ""); PreparedStatement pst = con.prepareStatement("SELECT patient_id, name, gender, contact, email, image_path FROM patients"); ResultSet rs = pst.executeQuery()) {
    ...
    private void filterAndSort() {
        ...
        Comparator<Object[]> comparator = switch (sort) {
            case "Sort by ID ↓" ->
                Comparator.comparing(o -> o[0].toString(), Comparator.reverseOrder());
            case "Sort by Name ↑" ->
                Comparator.comparing(o -> o[1].toString());
            case "Sort by Name ↓" ->
                Comparator.comparing(o -> o[1].toString(), Comparator.reverseOrder());
            default ->
                Comparator.comparing(o -> o[0].toString());
        };
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Patient Directory - Modern UI");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.setSize(960, 600);
            frame.setLocationRelativeTo(null);
            frame.add(new PatientDisplayModern());
